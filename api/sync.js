  ...
  if (req.method === 'POST' || req.method === 'PUT') {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        if (data.event) cachedEvent = data.event;
      ...
      return res.status(200).json({ success: true, count: (cachedAttendees || []).length, data: cachedAttendees, event: cachedEvent });
    } catch (e) {
      return res.status(400).json({ error: 'Invalid JSON body' });
    }
  }

  return res.status(200).json({ success: true, data: cachedAttendees || [], event: cachedEvent });
}
